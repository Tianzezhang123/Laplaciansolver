...
for k = 1:5
    ...
    p = 2*2^k;
    meshgrids(k) = n*p;
    [err,time]=ie_fmmsquare(n,occ,p);
    errors(k) = err;
    times(k) = time;
end

loglog(meshgrids, errors, 'm*-'); 
grid on;
title('Error of the FMM approach with square boundary');
...
function [err1,time]=ie_fmmsquare(n,occ,p,rank_or_tol,Tmax,near,store,symm)
